...
// 実行可能ファイルとしてパッケージに含まれるため、
// 外部のファイルに依存せずに実行することができる。
// ! // と go:embedの間にスペースは入れない
...
// JSTを付与してパースすることで日本時間で扱えるようにする
// ? この書き方全然分からん
/*
Int64やStringがアドレスを渡して書き換えるインターフェースになっているリスト10.4と異なり、
customFuncには戻り値や参照引数で更新された値を返すしくみがありません。そこで更新対象の
変数の参照を引数に取りつつ、クロージャを返す関数にすることで、見た目のきれいな実装にしています。
? クロージャが分からん
*/
...
func main() {
	...
	// Echo アプリケーションで /static/ パス以下の静的ファイルを提供するためのハンドラを設定
	staticFs, err := fs.Sub(static, "staic")
	if err != nil {
		log.Fatal(err)
	}
